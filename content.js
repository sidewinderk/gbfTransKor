...
async function InitList() {
	chrome.storage.local.get(
		['oTEXT', 'nTEXT', 'mTEXT', 'verboseMode', 'origin', 'imageswap', 'battleobserver'],
		function(result) {
...
function GetTranslatedText(node, csv, isStoryText = false) {
	if (node) {
		...
		if (passOrNot) {
			...
			translatedText = isStoryText
				? translate_StoryText(textInput, csv)
				: translate(textInput, csv);
			if (translatedText == '') transtextInput = textInput;
	...
	mutations.some(function(mutation) {
		...
		if (mutation.target.className.includes('prt-log-display')) {
			...
			chrome.storage.local.get(['extractMode', 'translateMode'], function(result) {
				if (result.extractMode) {
					if (kCheck.test(textmessage)) return;

					console.log('storyText_index:'+storyText_index);
					console.log('log length : '+LogLength);		
		...
		if (mutation.target.className.includes('txt-character-name')) {
			chrome.storage.local.get(['extractMode', 'translateMode'], function(result) {
				...
				if (tempName != '<span>&nbsp;</span>') {
					if (kCheck.test(tempName)) return;
					if (result.extractMode) {
